...
def analyze_pl_comparison(df: pd.DataFrame, params_list: list) -> dict:
    ...
    task_list 파라미터: 계정과목 / 구분(차변·대변·both) / 손익구분(매출·매출원가·판관비·
        제조원가·영업외수익·영업외비용) / 실행여부 / 관리단위(선택) / 비용구분(선택)
    결과: 손익구분마다 시트 1개(행=계정명, 열=1월~12월+합계)로 정리.
    손익구분을 안 적으면 구분(차변/대변)으로 매출·판관비만 추정해 채움 — 매출원가/
    제조원가/영업외수익/영업외비용으로 분류하려면 손익구분 값을 직접 적어야 함.
    (회사에 따라 계정 자체가 판관비/제조원가로 명확히 구분되면 손익구분에 각각 직접
    적어 별도 시트로 뽑으면 되고, samdong처럼 계정만으로는 판관/제조가 갈리지 않는
    회사는 손익구분에 일괄 '판관비'만 적고 아래 비용구분(Y) 옵션으로 원장의 실제
    판관/제조 값을 반영해 같은 시트 안에서 행을 나누는 쪽을 쓴다. 2026-09-07 blue sky
    피드백 — 처음엔 판관비/제조원가를 하나로 통합했다가, 계정 자체로 구분되는 회사가
    있어 다시 별도 카테고리로 되돌림.)
