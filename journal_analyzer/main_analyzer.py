...
# ── 25. 손익항목 월별 추이 ────────────────────────────────────────────────────
_PL_CATEGORIES = ['매출', '매출원가', '판관비/제조원가', '영업외수익', '영업외비용']
# 손익구분 입력 시 '판관비'/'제조원가'만 단독으로 적어도 통합 카테고리로 인식
# (2026-09-07 blue sky 요청 — 손익구분 단계에서 판관비/제조원가를 명확히 나누기
# 어려워 하나의 카테고리로 묶고, 실제 판관/제조 구분은 원장의 비용구분 컬럼을
# 그대로 쓰는 '비용구분' 옵션(Y)에 맡김)
_PL_CATEGORY_ALIASES = {'판관비': '판관비/제조원가', '제조원가': '판관비/제조원가'}

def analyze_pl_comparison(df: pd.DataFrame, params_list: list) -> dict:
    """25. 손익항목 월별 추이 (당기, 손익구분별 시트)
    task_list 파라미터: 계정과목 / 구분(차변·대변·both) / 손익구분(매출·매출원가·
        판관비/제조원가·영업외수익·영업외비용) / 실행여부 / 관리단위(선택) / 비용구분(선택)
    결과: 손익구분마다 시트 1개(행=계정명, 열=1월~12월+합계)로 정리.
    손익구분을 안 적으면 구분(차변/대변)으로 매출·판관비/제조원가만 추정해 채움 —
    매출원가/영업외수익/영업외비용으로 분류하려면 손익구분 값을 직접 적어야 함.
    판관비/제조원가는 굳이 미리 나누지 않고 하나의 카테고리로 묶음(현장에서 손익구분
    단계에 판관비인지 제조원가인지 명확히 가리기 어렵다는 요청, 2026-09-07) — 실제
    판관/제조 구분이 필요하면 아래 비용구분(Y) 옵션으로 원장 값을 그대로 반영해 같은
    시트 안에서 행을 나눈다.
    ...
    for p in params_list:
        ...
        if direction not in ('차변', '대변', 'both'):
            direction = '대변'
        cat = _nv(p.get('손익구분', ''))
        cat = _PL_CATEGORY_ALIASES.get(cat, cat)
        if cat not in _PL_CATEGORIES:
            cat = '매출' if direction == '대변' else '판관비/제조원가'
        unit_key = next((k for k in p.keys() if '관리단위' in str(k)), None)
