...
=======================================
sejoong(자체 개발 in-house 회계 프로그램) 분개장 전용 전처리.
...
날짜와 결합해도 복구 불가능한 수준(in-house 시스템에서 내려받아 저장하는
과정에서 이미 깨진 것으로 추정 — 사용자가 업로드 전 수기로 만들던 '회계번호'
컬럼도 단순히 이 손상된 전표번호를 일자와 이어붙인 것이라 손상이 그대로 남음).
