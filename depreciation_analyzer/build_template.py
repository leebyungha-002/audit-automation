...
def build():
    ...
    examples = [
        [
            ...
            "정액법", None, "당월", None, 45000000, 15000000, "제조", "예시 행 — 실제 자산으로 교체",
        ],
        [
            "FA-0002", "차량운반구", "예시) 업무용 승용차", "2024-07-01", 45000000, 0, 5,
            "정률법", 0.451, "익월", None, 0, 10147500, "판관", "예시 행 — 실제 자산으로 교체",
        ],
    ]
    for r, row in enumerate(examples, start=3):
        for c, val in enumerate(row, start=1):
            ...
            cell.border = border
            if COLUMNS[c - 1][1] == "취득일" and val:
                cell.number_format = "yyyy-mm-dd"
            if COLUMNS[c - 1][1] in ("취득원가", "잔존가치", "전기말 회사계상 감가상각누계액", "당기 회사계상 감가상각비"):
    ...
    lines = [
        ...
        "6. 회사계상액(대사용) — '당기 회사계상 감가상각비'를 채워두면, 앱이 재계산한 금액과 자동 비교해 차이를 표시합니다.",
        "   모르면 비워둬도 앱 실행에는 문제 없습니다(대사만 생략됨).",
        "7. 파일명 규칙: 이 템플릿을 복사해 'depreciation_<회사명>_information_fy<회계연도>.xlsx' 로 저장하세요.",
