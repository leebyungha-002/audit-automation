...
def build():
    ...
    # 예시행: 유형자산(정액법/정률법/손상차손/정부보조금), 투자부동산, 무형자산 각 케이스
    examples = [
        [
            "본사", "유형자산", "FA-0001", "기계장치", "예시) 사출성형기 1호", "2023-03-15", 120000000, 0, 8,
            "정액법", None, "당월", None, None, None, None, None, 45000000, 15000000, None, None,
            "제조", "예시 행 — 실제 자산으로 교체",
        ],
        [
            "본사", "유형자산", "FA-0002", "차량운반구", "예시) 업무용 승용차", "2024-07-01", 45000000, 0, 5,
            "정률법", 0.451, "익월", None, None, None, None, None, 0, 10147500, None, None,
            "판관", "예시 행 — 실제 자산으로 교체",
        ],
        [
            "제2공장", "유형자산", "FA-0003", "건물", "예시) CGU 손상평가 반영 자산", "2015-01-01", 500000000, 0, 40,
            "정액법", None, "당월", None, "2025-12-31", 80000000, None, None, None, None, None, None,
            "제조", "예시) 25년말 CGU 손상평가로 손상차손 인식 → 남은 기간(내용연수 그대로) 정액법 재상각",
        ],
        [
            "본사", "투자부동산", "FA-0004", "건물(투자)", "예시) 임대용 건물", "2019-05-01", 300000000, 0, 30,
            "정액법", None, "당월", None, None, None, None, None, None, None, None, None,
            "영업외비용", "예시 행 — 실제 자산으로 교체",
        ],
        [
            "제2공장", "무형자산", "FA-0005", "소프트웨어", "예시) ERP 라이선스", "2022-01-01", 60000000, 0, 5,
            "정액법", None, "당월", None, None, None, None, None, None, None, None, None,
            "판관", "예시 행 — 실제 자산으로 교체",
        ],
        [
            "본사", "유형자산", "FA-0006", "기계장치", "예시) 정부보조금 지원 설비", "2024-01-01", 120000000, 0, 5,
            "정액법", None, "당월", None, None, None, "기계장치(국고보조금)", 30000000, None, None, None, None,
            "제조", "예시) 취득원가 중 3천만원은 국고보조금으로 취득 — 상각과 동일 비율로 환입되어 감가상각비를 차감",
        ],
    ]
    for r, row in enumerate(examples, start=3):
        for c, val in enumerate(row, start=1):
            ...
            if COLUMNS[c - 1][1] in (
                "취득원가", "잔존가치", "손상차손 인식액", "정부보조금 수령액",
                "전기말 회사계상 감가상각누계액", "당기 회사계상 감가상각비",
                "전기말 회사계상 보조금잔액", "당기 회사계상 보조금환입액",
            ):
    ...
    lines = [
        ...
        "1-2. 자산분류: 유형자산/투자부동산/무형자산 중 선택(필수 아님, 비워두면 '(미분류)'로 요약표에 표시됨).",
        "     이 값을 기준으로 '요약표' 시트에 분류별 상각대상 자산수·기초/기말장부금액·사업장별 제조/판관비 감가상각비가 자동 집계됩니다.",
        "1-3. 취득원가 증감: 별도 입력란은 없습니다 — '취득일'이 당기(회계연도) 안이면 자동으로 '당기증가(신규취득)'로,",
        "     '처분일'이 당기 안이면 '당기감소(처분)'로 분류되어 출력물에 기초취득원가/당기증가/당기감소/기말취득원가로 나와서 표시됩니다.",
        "     처분된 자산은 처분월까지만 정상적으로 월할상각하고, 그 시점의 취득원가·감가상각누계액을 당기에 전액 제거해 기말잔액이 0으로 맞춰집니다.",
        ...
        "7. 정부보조금(선택) — 국고보조금 등을 받아 취득한 자산만 입력(해당 없으면 비워둠).",
        "   '정부보조금 계정명'은 회사마다 다른 실제 차감계정명을 그대로 적으면 됩니다(예: '기계장치(국)', '기계장치(정부보조금)', '기계장치(국고보조금)' 등 — 계산에는 쓰이지 않고 출력물 표시용).",
        "   '정부보조금 수령액'만 입력하면, 그 자산의 감가상각(정액법/정률법)과 정확히 같은 비율로 매월 자동 환입되어 감가상각비를 차감합니다(환입액이 크면 취득원가 대비 보조금 비율도 커짐).",
        "   출력물에는 정부보조금잔액(기초/당기환입/기말)과 '순 감가상각비(보조금차감후)'가 별도 컬럼으로 표시되고, 요약표의 사업장별 감가상각비도 이 순액 기준으로 집계됩니다.",
        "   기말장부가액도 취득원가-감가상각누계액-손상차손누계액-정부보조금잔액으로 계산됩니다.",
        "8. 회사계상액(대사용) — '당기 회사계상 감가상각비'/'당기 회사계상 보조금환입액' 등을 채워두면, 앱이 재계산한 금액과 자동 비교해 차이를 표시합니다.",
        "   모르면 비워둬도 앱 실행에는 문제 없습니다(대사만 생략됨).",
        "9. 파일명 규칙: 이 템플릿을 복사해 'depreciation_<회사명>_information_fy<회계연도>.xlsx' 로 저장하세요.",
