...
실행: python interest_expense_analysis.py [입력파일경로]
기본: python interest_expense_analysis.py dae_il/results/이자비용적정성.xlsx
"""
...
def main():
    ...
    input_path = (
        ...
        else root / 'dae_il' / 'results' / '이자비용적정성.xlsx'
    )
    if not input_path.exists():
        print(f'[오류] 파일 없음: {input_path}')
        sys.exit(1)

    output_path = input_path.parent / '이자비용분석결과.xlsx'
