...
설계 원칙: 일반기업회계기준 퇴직급여충당부채(퇴직금 추계액 방식) 검증.
  ...
  - 전기/당기 인원은 회사별로 별도 파일을 만드는 대신, 이 한 파일 안에 '당기인원정보'/'전기인원정보'
    두 시트로 나눠 입력받는다(회사가 매년 스냅샷으로 제공하는 인원현황을 그대로 붙여넣는 방식).
    당기 계산(추계액)은 '당기인원정보'만 사용하고, '전기인원정보'는 사번(없으면 성명) 기준으로 매칭해
    신규입사자/퇴사자 명단을 산출하는 데에만 쓰인다.
  - '중간정산일'은 재직 상태를 유지한 채 근속기간만 재기산되는 것(퇴사가 아님)이므로,
    재직일수 계산의 기산일(입사일 대신 중간정산일부터 기산)에만 반영된다.
  - '배수(임원)'는 임원퇴직금지급규정 등에 따라 배수를 적용하는 회사를 위한 항목으로,
    '부서' 값이 정확히 '임원'인 행에서만 읽어 최종 추계액에 곱한다(그 외 행은 항상 1배).
"""
import os
import openpyxl
...
# (그룹헤더, 상세헤더, 열너비) — '당기인원정보'/'전기인원정보' 두 시트가 동일한 구조를 공유한다.
COLUMNS = [
    ("인적사항", "사업장", 14),
    ("인적사항", "부서", 14),
    ("인적사항", "사번", 12),
...
# — '당기인원정보'/'전기인원정보' 두 시트에 걸쳐 계속재직자는 양쪽 모두, 신규입사자는 당기만,
...
EXAMPLES_CURRENT = [
    ...
    [
        ...
        "판관비", "2026-04-01", None,
        3200000, 0, None, "예시) 당기 중 신규입사 — '전기인원정보'에는 없어 요약표의 '신규입사자 명단'에 자동 표시됨",
    ],
    [
...
EXAMPLES_PRIOR = [
    ...
    [
        ...
        4100000, 0, None,
        "예시) 당기 중 퇴사 — '당기인원정보'에는 이 사번이 없어 요약표의 '퇴사자 명단'에 자동 표시됨",
    ],
]
...
def build():
    ...
    ws_current.title = "당기인원정보"
    ws_prior = wb.create_sheet("전기인원정보")
    ...
    notes = [
        "전기말 재무제표(또는 결산정산표)상 실제 계상액 — 앱의 당기말 재계산액과의 증감 비교용 참고값(계산에는 반영되지 않음)",
    ...
    lines = [
        ...
        "  감가상각비 앱과 달리 기초(전기말) 잔액을 신뢰하지 않습니다 — 당기말 잔액은 회사계상 입력값과",
        "  무관하게 재직일수·급여만으로 전액 독립 계산되며, 회사계상 전기말/당기말 잔액은 대사(비교)용으로만 쓰입니다.",
        "",
        "0. 시트 구성 — '당기인원정보'와 '전기인원정보' 두 시트로 나뉩니다(회사별 파일을 따로 만들 필요 없음).",
        "   '당기인원정보': 이번 결산기준일 현재 재직 중인 인원 전체. 퇴직금 추계액 계산에 실제로 쓰이는 시트입니다.",
        "   '전기인원정보': 직전 결산기준일 현재 재직 중이었던 인원 전체(회사가 작년에 준 인원현황을 그대로 붙여넣으면 됨).",
        "     이 시트는 급여 등 금액 정보를 계산에 쓰지 않고, 사번(없으면 성명) 기준으로 '당기인원정보'와 매칭해",
        "     신규입사자/퇴사자 명단을 만드는 데에만 사용됩니다.",
        "   ※ 사번이 두 시트에서 반드시 동일해야 정확히 매칭됩니다. 사번이 없으면 성명으로 매칭하니 동명이인에 주의하세요.",
        "",
        "1. 인원 1명 = 1행. 두 시트 모두 같은 컬럼 구조입니다. 계속 재직 중인 사람은 두 시트 모두에,",
        "   당기 신규입사자는 '당기인원정보'에만, 당기 중 퇴사한 사람은 '전기인원정보'에만 입력하면 됩니다.",
        "",
        "2. 재직정보",
        ...
        "   앱은 이 급여기준액을 연간환산해 1일평균임금 = 급여기준액 × 12 / 365 로 추정한 뒤,",
        "   퇴직금 추계액 = 1일평균임금 × 30일 × (재직일수 / 365) × 배수 로 인원별 추계액을 계산합니다.",
        "   재직일수 = 당기 결산기준일 − 기산일(중간정산일이 있으면 그 날짜, 없으면 입사일).",
        "   '배수(임원)': 임원퇴직금지급규정 등에 따라 배수를 적용하는 회사만 사용합니다.",
        "     '부서' 값이 정확히 '임원'인 행에서만 이 값을 읽어 적용합니다(오탈자·다른 부서명이면 무시되고 1배로 계산됨).",
        "     일반 직원 행은 이 칸에 값이 있어도 무시됩니다. 임원인데 배수를 비워두면 1배로 계산되고 비고에 안내가 남습니다.",
        "",
        "4. 신규입사자/퇴사자 명단 (요약표에 자동 산출)",
        "   신규입사자 = '당기인원정보'에는 있으나 '전기인원정보'에는 없는 사번(또는 성명).",
        "   퇴사자 = '전기인원정보'에는 있으나 '당기인원정보'에는 없는 사번(또는 성명).",
        "   '전기인원정보' 시트를 비워두면(또는 시트 자체를 지우면) 인원변동 명단만 생략되고 당기 계산은 정상 진행됩니다.",
        "",
        "5. '기준정보' 시트 — 전기말/당기말 회사계상 퇴직급여충당부채(제조원가분/판관비분) 4칸만 입력합니다.",
        "   두 값 모두 앱 재계산액과의 대사(비교)용 참고값일 뿐, 계산 자체에는 반영되지 않습니다.",
