...
def build():
    ...
    lines = [
        ...
        "   '당기정보': 이번 결산기준일 현재 재직 중인 인원 전체. 퇴직금 추계액 계산에 실제로 쓰이는 시트입니다.",
        "   '전기정보': 직전 결산기준일 현재 재직 중이었던 인원 전체(회사가 작년에 준 인원현황을 그대로 붙여넣으면 됨).",
        "     이 시트는 급여 등 금액 정보를 계산에 쓰지 않고, 사번(없으면 성명) 기준으로 '당기정보'와 매칭해",
