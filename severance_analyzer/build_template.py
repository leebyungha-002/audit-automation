...
설계 원칙: 일반기업회계기준 퇴직급여충당부채(퇴직금 추계액 방식) 검증.
  ...
  - '부서' 값이 정확히 '임원'인 행은 일반 직원과 다른 산식(소득세법상 임원 퇴직금 한도 산식)을 쓴다:
    [(급여기준액×근속연수 + 급여기준액×근속월/10) / 10] × 배수. '배수(임원)'는 이 행에서만 읽어 적용한다.
"""
import os
import openpyxl
...
COLUMNS = [
    ...
    ("임금정보", "상여금(연간, 원)", 14),
    ("임금정보", "배수(임원)", 10),
    ("대사용(선택)", "회사계상 퇴직금추계액(원)", 18),
    ("기타", "비고", 30),
]

MONEY_FIELDS = ("월평균급여(원)", "상여금(연간, 원)", "회사계상 퇴직금추계액(원)")
...
# '당기퇴사자' 시트 — 선택 입력. 퇴직금(퇴직연금) 지급 사유는 크게 두 가지다:
#   '실제퇴사' — 정말로 회사를 그만둔 경우(당기정보에서 빠짐)
#   '중간정산' — 재직 중 퇴직금을 미리 정산받은 경우(당기정보에 그대로 남아있는 게 정상)
# 이 둘을 구분하지 않으면 중간정산자가 "당기정보에도 존재함 — 확인 필요" 경고로 잘못 표시된다.
# '입사일'은 선택 항목으로, 사번/성명이 '전기정보'에서 매칭되지 않는 인원(예: 근속 1년 미만이라
# 애초에 전기 인원현황에 없었던 경우)의 근속기간을 판단하는 데 보조적으로 쓰인다.
LEAVER_COLUMNS = [("퇴사자 지급정보", "사번", 14), ("퇴사자 지급정보", "성명", 14),
                   ("퇴사자 지급정보", "실제지급액(원)", 16), ("퇴사자 지급정보", "지급사유(실제퇴사/중간정산)", 18),
                   ("퇴사자 지급정보", "입사일(선택)", 14), ("퇴사자 지급정보", "비고", 30)]
LEAVER_PAYMENT_REASON_OPTIONS = ["실제퇴사", "중간정산"]
LEAVER_EXAMPLES = [
    ["EMP-0999", "예시)최과장", 38500000, "실제퇴사", None,
     "예시) 전기정보에 있는 사번으로 매칭 — 전기말 추계액과 자동 대사됨. 사번 없으면 성명으로 매칭."],
    ["EMP-1004", "예시)박민수", 15000000, "중간정산", None,
     "예시) 재직 중 퇴직금 중간정산 — '당기정보'에 그대로 남아있는 게 정상이며, 요약표 '퇴사자 명단 대사'"
     " 표의 비고에 '중간정산'으로만 표시되고 '당기정보에도 존재함' 경고는 뜨지 않음",
     ],
]
...
EXAMPLES_CURRENT = [
    [
        ...
        4200000, 6000000, None, 44000000,
        "예시 행 — 실제 인원으로 교체(기존 재직자, 연간상여금 600만원 지급). "
        "'회사계상 퇴직금추계액'을 채우면 앱 계산값과 자동 대사(차이 유의적이면 강조)",
    ],
    [
        "본사", "경영지원팀", "EMP-1002", "예시)김영희", "대리",
        "판관비", "2021-11-15", None,
        3800000, 0, None, None, "예시 행 — 기존 재직자(상여금 없음, 회사계상액 모름 — 비워두면 대사만 생략)",
    ],
    [
        "본사", "영업팀", "EMP-1003", "예시)이철수", "사원",
        "판관비", "2026-04-01", None,
        3200000, 0, None, None, "예시) 당기 중 신규입사 — '전기정보'에는 없어 요약표의 '신규입사자 명단'에 자동 표시됨",
    ],
    [
        "제2공장", "생산2팀", "EMP-1004", "예시)박민수", "사원",
        "제조원가", "2018-06-01", "2023-06-30",
        3500000, 4200000, None, None,
...
EXAMPLES_PRIOR = [
    [
        "본사", "생산1팀", "EMP-1001", "예시)홍길동", "과장",
        "제조원가", "2019-03-02", None,
        4000000, 5500000, None, 37000000,
        "예시 행 — 당기와 동일 인원(전기 시점 급여). 이 시트의 급여는 계산에 쓰이지 않음(매칭용). "
        "'회사계상 퇴직금추계액'만 전기말 대사에 쓰임",
    ],
    [
        "본사", "경영지원팀", "EMP-1002", "예시)김영희", "대리",
        "판관비", "2021-11-15", None,
        3800000, 0, None, None, "예시 행 — 당기와 동일 인원",
    ],
    [
        "제2공장", "생산2팀", "EMP-1004", "예시)박민수", "사원",
        "제조원가", "2018-06-01", "2023-06-30",
        3500000, 4200000, None, None, "예시 행 — 당기와 동일 인원(중간정산 이력 포함)",
    ],
    [
        "본사", "임원", "EMP-2001", "예시)정대표", "대표이사",
        "판관비", "2015-01-01", None,
        10000000, 18000000, 2.0, None, "예시 행 — 당기와 동일 인원",
    ],
    [
...
def build():
    ...
    dv_reason = DataValidation(
        type="list", formula1=f'"{",".join(LEAVER_PAYMENT_REASON_OPTIONS)}"',
        allow_blank=True, showErrorMessage=True,
    )
    dv_reason.error = "실제퇴사 또는 중간정산 중 선택하세요."
    ws_leaver.add_data_validation(dv_reason)
    reason_col = get_column_letter(
        next(i for i, (_, name, _) in enumerate(LEAVER_COLUMNS, start=1) if name == "지급사유(실제퇴사/중간정산)")
    )
    dv_reason.add(f"{reason_col}3:{reason_col}300")
    ...
    lines = [
        ...
        "   급여기준액 = 월평균급여 + 상여금(연간) × 3 / 12  (임원 산식·화면 표시용. 상여금 최근 3개월분 상당액 가산)",
        "",
        "   [일반 직원] 근로기준법 평균임금 산식",
        "     1일평균임금 = (월평균급여 × 3 + 상여금(연간) / 4) / 92",
        "       (최근 3개월간 임금총액 ÷ 최근 3개월간 총일수(92일 근사) — 근로기준법상 평균임금 정의 그대로)",
        ...
        "     (평균급여 × 10% × 근속연수 × 배수 구조 — '10%'는 근속월 나머지항이 아니라 전체에 걸리는 계수입니다)",
        ...
        "   '회사계상 퇴직금추계액(원)' (선택): 감가상각비 앱의 '당기 회사계상 감가상각비'와 같은 용도입니다.",
        "     회사가 인원별로 이미 계산해둔 퇴직금 추계액(퇴직연금 명세, 인사시스템 등)을 알면 채워 넣으세요.",
        "     '당기정보'에 채우면 당기말 계산값과, '전기정보'에 채우면 전기말 계산값과 인별로 자동 대사되어",
        "     '인원별추계명세'/'전기인원별추계명세' 시트에 차이가 표시되고 유의차이는 노란색으로 강조됩니다.",
        "     원가구분별 총액 차이가 어디서 나는지(어느 개인 때문인지) 찾을 때 특히 유용합니다.",
        "     모르면 비워둬도 앱 실행에는 문제 없습니다(해당 인원만 대사가 생략됨).",
        "",
        ...
        "     - 자동 산출 명단에만 있으면: '당기퇴사자 시트에 지급액 입력 없음'.",
        "     - '당기퇴사자' 시트에만 있으면(전기정보 매칭 실패): 입력된 입사일(또는 전기정보 입사일) 기준",
        "       전기 결산기준일 근속기간이 1년 미만이면 '전기 결산기준일 기준 근속 1년 미만 — 전기말",
        "       퇴충설정대상 아님'(근로기준법상 계속근로기간 1년 미만은 퇴직금 지급 요건 미충족), 그 외에는",
        "       '전기정보에서 매칭되는 인원을 찾지 못함(확인 필요)'.",
        "     - 양쪽에 다 있는데 그 인원이 '당기정보'에도 그대로 남아있으면(현재도 재직 중으로 보이면):",
        "       '당기정보에도 그대로 존재함 — 실제 퇴사 여부 확인 필요'. 단, '당기퇴사자' 시트의 지급사유가",
        "       '중간정산'이면 이 경고 대신 그냥 '중간정산'으로 표시됩니다(재직 중 중간정산은 당기정보에",
        "       그대로 남아있는 게 정상이므로).",
        "     - 같은 인원이 '당기퇴사자' 시트에 두 번 이상 입력되면 '이중기입의심' 경고(동명이인일 수도 있음).",
        "",
        "5. '당기퇴사자' 시트 (선택) — 퇴직금(퇴직연금) 지급액을 알면 채워서 인별 대사를 추가로 받을 수 있습니다.",
        ...
        "   '지급사유(실제퇴사/중간정산)': 대한민국 퇴직금 지급 사유는 크게 '실제퇴사'와 '중간정산'(재직 중",
        "     퇴직금을 미리 정산받는 것) 두 가지입니다. 반드시 구분해서 선택하세요 — '중간정산'으로 표시하면",
        "     앱이 이 인원을 진짜 퇴사자로 취급하지 않고, '당기정보'에 그대로 남아있어도 경고를 띄우지 않습니다",
        "     (비워두면 '실제퇴사'로 간주해 기존처럼 처리됩니다).",
        "   같은 인원(사번/성명)이 두 번 이상 입력되면 '이중기입의심' 경고가 표시됩니다(동명이인일 수도 있으니 확인).",
        "   '입사일(선택)': '전기정보'에서 매칭이 안 되는 인원(예: 근속 1년 미만이라 애초에 전기 인원현황에",
        "     없었던 경우)에 한해, 여기 입사일을 적어두면 전기 결산기준일 기준 근속 1년 미만 여부를 판단하는 데",
        "     사용됩니다. 매칭이 되는 인원은 '전기정보'의 입사일을 그대로 쓰므로 비워둬도 됩니다.",
        "",
