...
설계 원칙: 일반기업회계기준 퇴직급여충당부채(퇴직금 추계액 방식) 검증.
  ...
  - 인원별 전기/당기 재직여부는 입사일과 결산기준일만으로 앱이 자동 판정한다. '중간정산일'은 재직 상태를
    유지한 채 근속기간만 재기산되는 것(퇴사가 아님)이므로 재직여부 판정에는 영향을 주지 않고, 재직일수
    계산의 기산일(입사일 대신 중간정산일부터 기산)에만 반영된다.
...
def build():
    ...
    examples = [
        ...
        [
            ...
            "판관비", "2026-04-01", None,
            3200000, 0, None, "예시) 당기 중 신규입사 — 전기 인원명단에는 없어 요약표의 '신규입사자 명단'에 자동 표시됨",
        ],
        [
    ...
    lines = [
        ...
        "   (전기에만 있었거나 당기에만 있는 사람도 모두 이 한 시트에 함께 입력 — 앱이 입사일로 자동 구분합니다).",
        "",
        "2. 재직정보",
        ...
        "     유지한 채 근속기간만 다시 기산되는 것(퇴사가 아님)이므로, 재직여부 판정에는 영향이 없고",
        "     재직일수 계산의 기산일이 입사일 대신 이 날짜로 바뀔 뿐입니다. 정산 이력이 없으면 비워둡니다.",
        "   ※ 이 템플릿에는 실제 퇴사일 필드가 없습니다 — 진짜 퇴사자까지 추적해야 하면 별도 요청하세요.",
        "",
        "3. 임금정보",
        ...
        "   전기 재직자 = 입사일 ≤ 전기말 / 당기 재직자 = 입사일 ≤ 당기말 (중간정산일은 이 판정에 영향 없음).",
        "   신규입사자 = 당기 재직자 중 전기 재직자가 아니었던 사람.",
        "   퇴사자 명단은 실제 퇴사일 필드가 없어 항상 비어 있습니다(위 1번 안내 참고).",
        "",
