...
설계 원칙: 연월차충당부채(잔여연차일수 × 1일통상임금 방식) 검증.
  ...
  - '기준정보' 시트의 '연차산정기준'(입사기준/회계기준)에 따라 근속연수를 세는 시점만 달라진다.
    입사기준: 개인별 입사기념일마다 근속연수 갱신. 회계기준: 전 직원이 회계연도 시작일에 일괄 갱신
    (입사연도는 비례연차). 미입력 시 '입사기준'으로 계산된다.
"""
import os
import openpyxl
...
def build():
    ...
    c3 = basis.cell(
        ...
        value="입사기준: 개인별 입사기념일마다 근속연수가 갱신되어 그 시점에 연차가 부여됩니다. "
              "회계기준: 전 직원이 회계연도 시작일에 일괄로 근속연수가 갱신되며, 입사연도에는 비례연차가 "
              "적용됩니다. 미입력 시 '입사기준'으로 계산됩니다.",
    )
    c3.border = border
