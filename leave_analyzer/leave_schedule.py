...
FORMULA_NOTE_LINES = [
    ...
    ("당기말 연차충당부채(계산) = 당기말 잔여연차일수(계산) × 1일 통상임금(입력)", False),
    ("", False),
...
def compute_prior_balances(전기_by_key: dict, 전기_anchor_start: date, 전기결산일: date, mode: str) -> dict:
    """'전기정보' 인원 각각의 전기 결산기준일 시점 연차충당부채를 사번(없으면 성명) 키로 반환.
    당기 인원별 표에서 '당기말 - 전기말' 차이를 인별로 계산하는 데 쓰인다."""
    return {k: compute_employee(e, 전기_anchor_start, 전기결산일, mode)["당기말충당부채"]
...
def compute_employee(emp: dict, anchor_start: date, anchor_end: date, mode: str) -> dict:
...
def build_schedule_table(employees: list, anchor_start: date, anchor_end: date,
                          신규입사_keys: set, 전기말_balances: dict, mode: str) -> pd.DataFrame:
    ...
    rows = []
    for e in employees:
        r = compute_employee(e, anchor_start, anchor_end, mode)
        원가구분 = _cost_type(e)
        key = _employee_key(e)
...
def build_prior_schedule_table(전기_employees: list, anchor_start: date, anchor_end: date, mode: str) -> pd.DataFrame:
...
def write_summary_sheet(ws, summary: dict, company: str, target_fy: str,
    ...
    ws.cell(row=1, column=1,
            value=(f"연월차충당부채 요약표 (회사: {company}, 회계연도: {target_fy}{period_note}, "
                   f"연차산정기준: {mode}, 전기말: {전기결산일}, 당기말: {당기결산일})")).font = Font(bold=True, size=13)
    for col in "ABCDEFG":
...
def save_results(df: pd.DataFrame, output_path: str, company: str, target_fy: str,
    ...
    prior_df = build_prior_schedule_table(전기_employees, 전기_anchor_start, 전기결산일, mode)
...
def main():
    ...
    df = build_schedule_table(당기_employees, 당기_anchor_start, 당기결산일,
                               matched["신규입사_keys"], 전기말_balances, mode)
