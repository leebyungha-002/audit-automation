...
def build():
    ...
    rows = [
        ("당기말 기준일(결산기준일, 선택)", None,
         "직접 입력하면 이 날짜를 당기말 기준일로 사용합니다(파일명의 fy<연도>·실행 시 --fiscal-month/"
         "--fiscal-year 옵션보다 우선). '연령분석표'/'분기말잔액' 시트에서 이 날짜와 정확히 일치하는 '기준일' "
         "행이 당기말 데이터로 쓰입니다. 비워두면 기존 방식대로 파일명(fy<연도>)과 --fiscal-month 옵션으로 "
         "자동 계산됩니다(옵션도 안 주면 기본값 12월말).", "date"),
        ("상장구분(상장/비상장)", "비상장",
    ...
    basis.cell(row=r + 1, column=1,
               value="※ 결산기준일(당기말)은 위 '당기말 기준일'에 직접 입력하거나, 비워두면 실행 시 파일명"
                     "(fy<연도>)과 --fiscal-month 옵션으로 다른 계정 검증앱과 동일한 규칙으로 자동 계산됩니다"
                     "(옵션도 안 주면 12월말). 어느 쪽이든 '연령분석표'/'분기말잔액' 시트에서 그 날짜와 정확히 "
                     "일치하는 '기준일' 행을 당기말 데이터로 사용합니다.").font = \
        Font(italic=True, color="808080")
    ...
    lines = [
        ...
        "   자동으로 표시됩니다.",
        "",
        "5. 결산기준일(당기말)은 '기준정보'의 '당기말 기준일'에 직접 입력할 수 있습니다(입력하면 파일명·",
        "   --fiscal-month/--fiscal-year 옵션보다 우선 적용). 비워두면 기존처럼 파일명의 'fy<연도>'와",
        "   --fiscal-month 옵션(기본 12월)으로 다른 계정 검증앱과 동일한 규칙으로 자동 계산됩니다. 어느 쪽이든",
        "   '연령분석표'/'분기말잔액' 시트에서 그 날짜와 정확히 일치하는 '기준일' 행을 당기말로 사용합니다",
        "   (일치하는 행이 없으면 그 이하 날짜 중 가장 최근 것을 당기말로 간주하고 경고를 표시합니다).",
        "",
