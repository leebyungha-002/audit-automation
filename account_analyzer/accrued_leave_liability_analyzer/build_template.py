...
COLUMNS = [
    ...
    ("연차현황", "당기 연차사용일수(일)", 16),
    ("연차현황", "1일 통상임금(원)", 14),
    ("대사용(선택)", "회사계상 기말 연차일수(일)", 18),
    ("대사용(선택)", "회사계상 기말 연차충당부채(원)", 20),
    ("기타", "비고", 30),
]

MONEY_FIELDS = ("1일 통상임금(원)", "회사계상 기말 연차충당부채(원)")
DAY_FIELDS = ("기초 이월연차잔여일수(일)", "당기 연차사용일수(일)", "회사계상 기말 연차일수(일)")
DATE_FIELDS = ("입사일",)
...
EXAMPLES_CURRENT = [
    [
        "본사", "생산1팀", "EMP-1001", "예시)홍길동", "과장",
        "제조원가", "2019-03-02",
        5, 3, 120000, 17, 600000,
        "예시 행 — 실제 인원으로 교체(기존 재직자). 기초 이월연차잔여일수(전기말 이월분) 5일 + 당기 부여일수(계산)"
        " − 당기 사용 3일로 당기말 잔여가 자동 계산됨. '회사계상 기말 연차일수'·'회사계상 기말 연차충당부채'를"
        " 채우면 앱 계산값과 각각 자동 대사(일수 차이·금액 차이 둘 다 확인 가능)",
    ],
    [
        "본사", "경영지원팀", "EMP-1002", "예시)김영희", "대리",
        "판관비", "2021-11-15",
        2, 10, 110000, None, None, "예시 행 — 기존 재직자(회사계상 일수·금액 모름 — 비워두면 대사만 생략)",
    ],
    [
        "본사", "영업팀", "EMP-1003", "예시)이철수", "사원",
        "판관비", "2026-06-01",
        0, 0, 100000, None, None,
...
EXAMPLES_PRIOR = [
    [
        "본사", "생산1팀", "EMP-1001", "예시)홍길동", "과장",
        "제조원가", "2019-03-02",
        4, 4, 118000, 15, 580000,
        "예시 행 — 당기와 동일 인원(전기 시점 데이터). 이 시트의 '기초 이월연차잔여일수'는 전전기말 이월분",
    ],
    [
        "본사", "경영지원팀", "EMP-1002", "예시)김영희", "대리",
        "판관비", "2021-11-15",
        5, 13, 108000, None, None, "예시 행 — 당기와 동일 인원",
    ],
    [
        "제2공장", "생산2팀", "EMP-1004", "예시)박민수", "사원",
        "제조원가", "2018-06-01",
        6, 14, 103000, None, None, "예시 행 — 당기와 동일 인원",
    ],
    [
        "본사", "생산1팀", "EMP-0999", "예시)최과장", "과장",
        "제조원가", "2012-02-01",
        10, 10, 100000, None, None,
...
def build():
    ...
    # 당기퇴사자 시트 — 선택 입력. 채우면 퇴사자별 전기말 연차충당부채 vs 실제연차지급액 대사가 요약표에 추가된다.
    ...
    lines = [
        ...
        "",
        "   '회사계상 기말 연차일수(일)' (선택): 회사 연차관리대장상 그 시점 잔여연차일수를 알면 채워",
        "     넣으세요. 앱이 계산한 '당기말/전기말 잔여연차일수(계산)'과 인별로 자동 대사되어 일수 차이가",
        "     표시됩니다 — 통상임금 오류와 무관하게 '일수 자체'가 맞는지만 따로 확인하고 싶을 때 유용합니다.",
        "   '회사계상 기말 연차충당부채(원)' (선택): 회사가 인원별로 이미 계산해둔 연차충당부채(금액)를",
        "     알면 채워 넣으세요. '당기정보'에 채우면 당기말 계산값과, '전기정보'에 채우면 전기말 계산값과",
        "     인별로 자동 대사되어 '인원별추계명세'/'전기인원별추계명세' 시트에 일수차이·금액차이가 각각",
        "     표시되고 유의차이는 노란색으로 강조됩니다. 둘 다 모르면 비워둬도 앱 실행에는 문제 없습니다.",
        "",
        ...
        "   '사번'(없으면 성명) + '실제연차지급액(원)'만 입력하면, 앱이 '전기정보' 시트에서 같은 사번(성명)을",
        "   찾아 그 사람의 전기말 연차충당부채(계산)와 자동으로 비교(대사)해 요약표에 표시합니다. '실제연차",
        "   지급액'은 퇴직금 등 다른 정산 항목을 뺀 연차(미사용연차수당) 정산분만 입력해야 합니다 — 전기말",
        "   연차충당부채(계산)와 1:1로 직접 비교되는 값이라, 퇴직금까지 섞어 넣으면 차이가 왜곡됩니다.",
        "   사업장/부서/직급 등은 '전기정보'에서 그대로 가져오므로 다시 입력할 필요가 없습니다. 모르는 퇴사자는 행을 비워두면",
