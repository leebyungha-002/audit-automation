...
발생일(익년 1/1 등)은 인식 시점과 무관)를 따른다:
  ...
  [입사기준] 개인별 입사기념일마다 근속연수가 갱신된다. 당기 회계기간 중 도래하는 입사기념일마다
    ...
    (경과개월수 = 직전 기념일부터 결산기준일까지 달력월 개월수, 양끝 포함).
    예) 5/31 입사자, 결산기준일 12/31, 직전 기념일 그 해 5/31 → 경과 8개월 →
    다음 근속연수 부여일수 × 8/12 만큼을 그 해 전액부여분 위에 추가로 얹는다.
    ...
    if mode == "회계기준":
        ...
        if 입사일 > anchor_start:
            ...
            if 직전기념일 is not None:
                ...
                # 이후 결산일까지 경과한 개월수만큼, 다음 기념일에 부여될 예정 연차를 미리 비례
                # 인식한다. 예) 5/31 입사자, 직전기념일 6/1, 결산일 12/31 → 7개월 경과
                # → 다음 근속연수(+1년) 부여일수 × 7/12 를 당기부여일수에 가산.
                다음근속연수 = 근속연수_표시 + 1
                다음부여일수 = _entitlement_by_tenure(다음근속연수)
                경과개월수 = _months_between_inclusive(직전기념일, anchor_end)
